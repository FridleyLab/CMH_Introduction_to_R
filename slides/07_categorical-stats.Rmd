...
subtitle: "Chi-Squared, Fishers Exact, and Survival with Kaplan Meier"
author: 
  - Alex Soupir
  - Zachary Thompson 
...
***Time to last follow-up is simulated for those who had not died. Do not interpret the results as if they are real.***
...
target_p2 %>% 
  select(days_to_death, vital_status) %>%
  mutate(event = case_when(vital_status == "Alive" ~ 0,
                           TRUE ~ 1)) %>%
  head() %>% data.frame()
```
...
km_fit <- survfit(Surv(days_to_death, event) ~ 1, data=target_p2)
...
km_fit_gender <- survfit(Surv(days_to_death, event) ~ gender, data=target_p2)
...
To extract model statistics we can fit using `coxph` instead of `survfit`

```{r, echo = TRUE, fig.show = "hide", eval = FALSE}
km_fit_gender_cph <- coxph(Surv(days_to_death, event) ~ gender, data=target_p2)
summary(km_fit_gender_cph)
```
...
```{r comment=NA, results = "asis", echo = FALSE}
km_fit_gender_cph <- coxph(Surv(days_to_death, event) ~ gender, data=target_p2)
dat = summary(km_fit_gender_cph) %>% capture.output()
dat_prepped = prep_summary(dat)
...
The p-value fo r `gendermale` is greater than larger than our typical 0.05 indicating there is not a significant difference between gender survival times.
